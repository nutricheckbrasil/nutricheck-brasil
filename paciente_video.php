 ...
 * URL: /paciente_video.php?token={token}
 * 
 * Sistema completo de entrevista pré-anestésica com vídeos e perguntas
 * Integrado com validação de token e salvamento de respostas
 */
...
try {
    ...
    $questionarioPercentual = (int) ($paciente['questionario_percentual'] ?? 0);
    $questionarioVideosRespondidos = (int) ($paciente['questionario_videos_respondidos'] ?? 0);
    $questionarioTotalVideos = (int) ($paciente['questionario_total_videos'] ?? 0);
    $questionarioFinalizado = (
        ($questionarioStatus && $questionarioStatus === 'completo') ||
        ($paciente['status'] ?? '') === STATUS_QUESTIONARIO_RESPONDIDO ||
        $questionarioPercentual >= 100 ||
        ($questionarioTotalVideos > 0 && $questionarioVideosRespondidos >= $questionarioTotalVideos)
    );
    ...
    if ($questionarioFinalizado) {
        ...
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Questionário já respondido</title>
            <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
        </head>
        <body class="bg-light">
            <div class="container py-5">
                <div class="alert alert-success">
                    <h4 class="mb-3"><i class="bi bi-check-circle"></i> Questionário já concluído</h4>
                    <p>O link informado já foi utilizado e o questionário foi finalizado. Caso precise realizar alguma atualização, entre em contato com o seu nutricionista.</p>
                </div>
            </div>
        </body>
        </html>
    ...
    // Dados do paciente para JavaScript
...
// Estrutura de vídeos e perguntas
$videos = [
    [
        'id' => 'video_1',
...
<body>
    ...
    <script>
        ...
        async function saveResponseToDatabase(response) {
            try {
                ...
                formData.append('video_title', response.videoTitle);
                formData.append('question_id', response.questionId || '');
                formData.append('question_index', response.questionIndex);
                formData.append('question_text', response.questionText);
                formData.append('answer', typeof response.answer === 'object' ? JSON.stringify(response.answer) : response.answer);
                formData.append('type', response.type);
                formData.append('timestamp', response.timestamp);
                
                const saveResponse = await fetch('ajax/save_video_response.php', {
                    method: 'POST',
                    body: formData
                });
        ...
        // Modificar a função respondQuestion para salvar no banco
        const originalRespondQuestion = respondQuestion;
        respondQuestion = function(answer) {
            const result = originalRespondQuestion.call(this, answer);
            
            // Salvar última resposta no banco
            if (responses.length > 0) {
                const lastResponse = responses[responses.length - 1];
                saveResponseToDatabase(lastResponse);
            }
            
            return result;
        };
        
        // Função para marcar entrevista como concluída no banco
        async function finishInterviewInDatabase() {
            try {
                const formData = new FormData();
                formData.append('action', 'finish_interview');
                formData.append('paciente_id', <?= $paciente_id ?>);
                formData.append('token', '<?= $token ?>');
                formData.append('total_responses', responses.length);
                
                const finishResponse = await fetch('ajax/finish_video_interview.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                let result = null;
                try {
                    result = await finishResponse.json();
                } catch (parseError) {
                    console.error('Erro ao interpretar resposta ao finalizar entrevista:', parseError);
                }
                
                if (!finishResponse.ok || !result || !result.success) {
                    const errorMessage = result && result.error
                        ? result.error
                        : 'Não foi possível finalizar a entrevista. Tente novamente.';
                    updateStatus(errorMessage, 'error');
                    return;
                }
                
                if (result.message) {
                    updateStatus(result.message, result.questionario?.status === 'completo' ? 'success' : 'warning');
                }
                
                if (result.questionario && result.questionario.status !== 'completo') {
                    const pendentes = result.questionario.videos_pendentes || [];
                    const nomesPendentes = pendentes.map(item => item.title).join(', ');
                    const faltantes = result.questionario.total_videos - result.questionario.videos_respondidos;
                    const detalhe = nomesPendentes ? ` (${nomesPendentes})` : '';
                    updateStatus(`⚠️ Ainda faltam ${faltantes} vídeo(s) para concluir o questionário${detalhe}.`, 'warning');
                }
            } catch (error) {
                console.error('Erro ao finalizar entrevista:', error);
                updateStatus('Erro inesperado ao finalizar a entrevista. Tente novamente.', 'error');
            }
        }
        
        // Adicionar botão para ir ao consentimento ao finalizar
        const originalFinishTriagem = finishTriagem;
        finishTriagem = function() {
            originalFinishTriagem.call(this);
            
            // Marcar como concluído no banco
            finishInterviewInDatabase();
            
            // Adicionar botão para consentimento
            setTimeout(() => {
                const consentButton = document.createElement('button');
                consentButton.className = 'response-btn btn-submit';
                consentButton.innerHTML = '<i class="fas fa-arrow-right"></i> Continuar para Termo de Consentimento';
                consentButton.style.marginTop = '20px';
                consentButton.style.width = '100%';
                consentButton.style.maxWidth = '600px';
                consentButton.onclick = function() {
                    window.location.href = '<?= $link_consentimento ?>';
                };
                
                responseSection.appendChild(consentButton);
            }, 500);
        };
