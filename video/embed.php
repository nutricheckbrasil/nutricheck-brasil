 ...
 * Integrado com api_video.php do sistema NutriCheck
...
try {
    ...
    if (file_exists($app_path . '/config/database.php')) {
        ...
        $perguntas = $db->fetchAll($sql, [$video_id]);
...
// Tentar múltiplos campos para URL do vídeo (compatibilidade)
$video_url = '';
if (isset($video['file_path']) && !empty($video['file_path'])) {
    $video_url = $video['file_path'];
} elseif (isset($video['url_video']) && !empty($video['url_video'])) {
    $video_url = $video['url_video'];
} elseif (isset($video['video_url']) && !empty($video['video_url'])) {
...
<body>
    ...
    <script>
        ...
        // Mostrar feedback (SEM MOSTRAR ERRO)
        function showFeedback(correta, respostaCorreta, explicacao) {
            // NÃO mostrar feedback de erro, apenas continuar o vídeo
            if (!correta) {
                // Se resposta incorreta, apenas continua sem mostrar mensagem
                continueButton.style.display = 'block';
                return;
            }
            
            // Apenas mostrar feedback se for resposta correta
            feedbackMessage.style.display = 'block';
            feedbackMessage.className = 'feedback-message success';
            
            let html = '<i class="bi bi-check-circle-fill"></i>';
            html += '<strong>Resposta correta!</strong>';
